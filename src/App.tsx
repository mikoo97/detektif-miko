...
import AskDetective from './components/AskDetective';
import { defaultScenarios, Scenario } from './data/scenarios';

export default function App() {
  const [gameState, setGameState] = useState<'home' | 'playing' | 'score' | 'admin' | 'ask'>('home');
  ...
  return (
    <div className="min-h-screen bg-sky-100 font-sans text-slate-800 overflow-hidden">
      {gameState === 'home' && <Home onStart={startGame} onAdmin={() => setGameState('admin')} onAsk={() => setGameState('ask')} />}
      ...
      {gameState === 'ask' && <AskDetective onBack={() => setGameState('home')} />}
    </div>
  );
}
